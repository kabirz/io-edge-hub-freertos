...
int main(void)
{
    ...
    /* addr 5000: FC06 无 FP 分支 (Zephyr 同款), 整数回调越界 -> 0x02 */
    check_exc((uint8_t[]){0x06, 0x13, 0x88, 0x00, 0x01}, 5, 0x06, 0x02);
    ...
    check_exc((uint8_t[]){0x10, 0x00, 0x01, 0x00, 0x02, 0x04, 0x00, 0x05}, 8,
              0x10, 0x03);
    /* 整除规则 (Zephyr num_bytes/reg_qty != 2): qty=2/nbytes=5, 5/2=2
     * -> 通过, 写 2 个寄存器, 尾部多余字节 0x00 忽略 */
    check_rsp((uint8_t[]){0x10, 0x00, 0x01, 0x00, 0x02, 0x05, 0x00, 0x05, 0x00, 0x06, 0x00}, 11,
              (uint8_t[]){0x10, 0x00, 0x01, 0x00, 0x02}, 5);
    TEST_EQ_INT(get_holding_reg(0x01), 5);
    TEST_EQ_INT(get_holding_reg(0x02), 6);
    /* 整除规则拒: qty=2/nbytes=3 (3/2=1), dlen-5==nbytes -> 0x03,
     * 且不写任何寄存器 */
    check_exc((uint8_t[]){0x10, 0x00, 0x01, 0x00, 0x02, 0x03, 0x00, 0x05, 0x06}, 9,
              0x10, 0x03);
    TEST_EQ_INT(get_holding_reg(0x01), 5);
    /* FP 扩展区 -> 0x01 */
