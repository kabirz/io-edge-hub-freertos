...
/* 合法时间戳范围门 (纯函数, host 测试): [2000-01-01, 2100-01-01] 闭
 * 区间 UTC。越界值 (主站只写低 16 位、高字为 0 -> 1970 年等) 直接拒绝 */
bool ts_in_range(time_t t);

/* 设置 RTC + epoch 缓存 (Modbus 0x0E/0x0F 写低位或 UDP SET_TIME 调用)。
 * 返回 true=成功; false=参数越界或 RTC 写失败 (不动缓存, Zephyr 版
 * rtc_set_time+clock_settime 的合并语义) */
 ...
 * 默认 2020-01-01 00:00:00, 之后由 VBAT 维持 */
