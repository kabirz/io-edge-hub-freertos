...
struct mb_conn {
	...
	uint16_t rx_len;
	uint16_t pending_len;  /* ERR_MEM 暂存字节数, 0 = 无 */
	uint32_t t_last_ms;    /* 半帧超时基准 */
	uint8_t rx[MBTCP_ADU_RX_MAX];
	uint8_t tx_pending[MBTCP_ADU_TX_MAX];
};
...
/* 目标帧长: 6B 头 + MIN(MBAP length,256) (与 mbtcp_adu 钳制一致) */
...
static bool process_frame(struct mb_conn *c)
{
	...
	if (rsp != 1) {
		return true; /* 广播/非法 proto 静默 (判定在 mbtcp_adu) */
	}
			...
			err_t err)
{
	...
	for (struct pbuf *q = p; q != NULL; q = q->next) {
		...
		while (rem > 0) {
			...
			rem -= n;
			c->t_last_ms = now_ms();
			/* 8B 只是头就绪, 须收满 frame_need 才是完整 ADU */
...
static err_t mb_accept_cb(void *arg, struct tcp_pcb *newpcb, err_t err)
{
	...
	tcp_poll(newpcb, mb_poll_cb, 1); /* 单位 ~0.5s */
