 ...
 *     未知命令 (含 0x01-0x06 固件升级) 返回 0 静默
 *   - FACTORY_RESET 两步确认 (5s 窗): 计时起点 0, 开机 5s 内
 *     (含恰好 5000ms) 的第一条命令即视为确认步, 单命令直接执行
 *     (见 udp_factory_reset 注释)
 *
 * 应答契约 (见 udp_cfg_reboot_pending): 本层只 擦除 -> 写应答 -> 置
 * 重启待办标志; 传输层把应答 sendto 上线之后才 history_sync() +
 * io_reboot_cold() (顺序: 应答 -> 刷历史 -> 重启, 100ms 延时在
 * io_reboot_cold 实现内), 保证 [0x19][01] 先上线 -- 在本层直接重启
 * 会把应答一起带走。config_store_erase_all() 无返回值: ok 恒 1。
 */
...
/* [cmd][ok] 两字节应答; cap 不足返回 0 静默 (调用方缓冲 64B,
 * 不可达的防御路径) */
...
/* 确认判据是 "(now - pending) > 5000" 才算首步。pending 起点 0 ->
 * 开机 5s 内 (含恰好 5000ms) 的第一条命令落入确认分支直接执行 =
 * 单命令复位的怪癖, 有意保留。uint32 减法对回绕安全 (真实流逝
 * < 2^32 ms 时差值正确)。 */
...
static uint16_t udp_factory_reset(uint8_t *reply, uint16_t cap, uint8_t cmd)
{
	 ...
	 * 传输层必须先把 [0x19][01] sendto 上线, 再按标志执行 (顺序:
	 * 应答 -> 刷历史 -> 重启; 延时在 io_reboot_cold 实现内), 否则
	 * 应答随重启丢失。 */
		     ...
		     uint8_t *reply, uint16_t cap)
{
	...
	case UDP_CMD_GET_VERSION: {
		/* 返回版本字符串 "v<major>.<minor>.<patch>_<git_hash>"
		 * (无尾 NUL)。局部 const 数组中转 FW_GIT_VERSION, 避免
		 * MSVC 宏展开问题。 */
